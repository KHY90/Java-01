...
public class FinalFieldTest {
    ...
    *  6. static 매소드 : 매소드 재작성(overriding) 불가
    *  7. 클래스 : 상속 불가능
    * */

    // 아래와 같이 상수 필드의 값을 할당하지 않으면 초기값이 0으로 고정되어 에러가 발생
//    private final int NON_STATIC_NUM;
    ...
    /*
    *  생성자를 이용한 초기화는 불가능하다.
    *  생성자는 인스턴스가 생성되는 시점에 호출이되기 때문에 그 전에는 초기화가 일어나지 못한다.
    *  하지만 static은 프로그램이 start 될 때 할당되기 때문에 초기화가 되지 않은 상태로 선언된 것과 동일하여
